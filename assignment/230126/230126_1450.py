...
def highest_entry(entry_record):
    c = Counter(entry_record)
    return list(c.most_common(3))

def mismatch_entry(entry_record, exit_record):
    entry_c = Counter(entry_record)
    exit_c = Counter(exit_record)
    entry_c.subtract(exit_c)
    mismatches = entry_c.items()

    answer = []
    for mismatch in mismatches:
        if mismatch[1] != 0:
            answer.append(mismatch[0])
...
print(highest_entry(entry_record))
print(mismatch_entry(entry_record, exit_record))